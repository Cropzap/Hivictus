...
import { motion, useScroll, useTransform, AnimatePresence } from 'framer-motion'; // Import AnimatePresence for exit animations
import { Link } from 'react-router-dom';
// Using lucide-react icons for a modern, professional look fitting iOS style
import { Sparkles, BarChart2, Shield, Users, ArrowRight, CheckCircle, Leaf, Download, X } from 'lucide-react'; // Import X for close button

const AboutUs = () => {
  // Ref for the main scroll container for parallax effects
  const scrollRef = useRef(null);
  // State to control the visibility of the seller application popup
  const [showSellerAppPopup, setShowSellerAppPopup] = useState(false);

  // Parallax for the seller app image within its section
  const { scrollYProgress: sellerAppScrollProgress } = useScroll({
    target: scrollRef, // Using the main scroll container as target
    offset: ['start end', 'end start'], // Start when it enters, end when it leaves
  });

  const appImageY = useTransform(sellerAppScrollProgress, [0, 1], ['-10%', '10%']); // Subtle vertical float
  const appImageScale = useTransform(sellerAppScrollProgress, [0, 1], [0.95, 1.05]); // Subtle scale in/out

  // Animation variants for sections to slide in and fade up
  const sectionVariants = {
    hidden: { opacity: 0, y: 50 },
    visible: {
      opacity: 1,
      y: 0,
      transition: { duration: 0.7, ease: 'easeOut' }, // Smooth cubic-bezier easing
    },
  };

  // Animation variants for sub-elements (e.g., list items, smaller text blocks)
  const itemVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: 'easeOut' } },
  };

  // Card specific hover and whileInView animations with iOS spring physics
  const cardVariants = {
    hidden: { opacity: 0, y: 40, scale: 0.95 },
    visible: {
      opacity: 1, y: 0, scale: 1,
      transition: { type: "spring", stiffness: 120, damping: 14 } // Fine-tuned spring for iOS feel
    },
    hover: { scale: 1.03, boxShadow: "0 12px 24px rgba(0,0,0,0.1)", transition: { type: "spring", stiffness: 300, damping: 20 } },
    tap: { scale: 0.97 }
  };

  // Variants for the seller application popup
  const popupVariants = {
    hidden: { x: '100%', y: '100%', opacity: 0, scale: 0.8 }, // Start from bottom right, scaled down
    visible: {
      x: '0%', y: '0%', opacity: 1, scale: 1,
      transition: {
        type: "spring",
        stiffness: 100, // Make it a bit stiffer for a snappier popup
        damping: 15,
        mass: 0.5
      }
    },
    exit: { // Animation when the popup is closing
  ...
  return (
    <div ref={scrollRef} className="min-h-screen bg-white font-sans text-gray-800">
      {/* Fixed Header: Cropzap Logo Only */}
      

      {/* Main content container */}
      <main className="container mx-auto px-4 py-8 sm:px-6 lg:px-8 max-w-7xl pt-28 pb-12"> {/* pt-28 to clear fixed header */}

        {/* Hero Section - A clean welcome */}
        <motion.div
          className="bg-white rounded-3xl shadow-xl p-8 md:p-12 mb-16 border border-gray-100 text-center"
        ...
        {/* Dedicated Seller Application Download Section (retained as a full section) */}
        <motion.section
          className="bg-white rounded-3xl shadow-xl p-8 md:p-12 mb-16 border border-gray-100 flex flex-col lg:flex-row items-center justify-between gap-10"
          variants={sectionVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.2 }}
        >
          ...
          <div className="flex flex-col lg:flex-row items-center lg:items-start gap-8 lg:w-2/3">
            <motion.div
              className="relative w-48 h-auto flex-shrink-0"
              style={{ y: appImageY, scale: appImageScale }} // Apply parallax to image container
            ...
            >
              <img
                src="https://www.cropzap.com/logo/logo_bg.png" // Path to your uploaded image
            ...
            <div className="text-center lg:text-left">
              <motion.h2
                className="text-3xl sm:text-4xl font-bold text-green-800 mb-3 leading-tight" // Slightly reduced size
          ...
          >
            <motion.a
              href="/download-seller-app" // Link for download
              className="bg-green-600 hover:bg-green-700 text-white font-semibold py-4 px-10 rounded-full shadow-lg
                         flex items-center justify-center space-x-3 transition-all duration-300 ease-in-out
            ...
            >
              ...
              <Download className="w-6 h-6 ml-2 relative z-10" />
        ...
        >
          ...
          <motion.p
            className="text-md text-gray-600 mb-4 leading-relaxed"
            variants={itemVariants}
          ...
          <motion.p
            className="text-md text-gray-600 leading-relaxed italic"
            variants={itemVariants}
            transition={{ delay: 0.4 }}
          >
        ...
        <motion.section
          className="bg-white rounded-3xl shadow-xl p-8 md:p-12 mb-16 flex flex-col md:flex-row items-center gap-8 border border-gray-100"
          variants={sectionVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.2 }}
        >
          <div className="w-full md:w-1/3 flex justify-center">
            <motion.img
              src="https://placehold.co/200x200/edf2f7/4a5568?text=Founder" // Placeholder for Founder's image with lighter background
              alt="Dharunkumar, Founder & CEO of Cropzap"
              className="rounded-full w-48 h-48 object-cover border-4 border-green-200 shadow-lg"
          ...
          <div className="w-full md:w-2/3 text-center md:text-left">
            <h3 className="text-3xl font-bold text-green-800 mb-4">Founder's Message</h3>
            <p className="text-lg font-semibold text-gray-700 mb-2">I'm Dharunkumar, the Founder and CEO of Cropzap Agri Networks.</p>
            <motion.p
              className="text-md text-gray-600 leading-relaxed"
              variants={itemVariants}
              transition={{ delay: 0.2 }}
            ...
            <motion.p
              className="text-md text-gray-600 leading-relaxed mt-3"
              variants={itemVariants}
              transition={{ delay: 0.4 }}
            >
        ...
        <motion.section
          ...
          viewport={{ once: true, amount: 0.2 }}
        >
          <h3 className="text-3xl font-bold text-green-800 text-center mb-10">Our Key Values</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: <Sparkles size={36} className="text-yellow-500" />, title: 'Technology Empowerment', desc: 'Empowering through technology and data-driven solutions' },
              { icon: <BarChart2 size={36} className="text-blue-500" />, title: 'Economic Growth', desc: 'Driving economic growth by supporting India\'s 5 trillion dollar goal' },
              { icon: <Shield size={36} className="text-purple-500" />, title: 'Sustainable Practices', desc: 'Commitment to sustainable and innovative agricultural practices' },
              { icon: <CheckCircle size={36} className="text-green-500" />, title: 'Food Security', desc: 'Enhancing food security through advanced agricultural models' },
            ].map((value, index) => (
              <motion.div
                key={index}
                className="bg-gray-50 rounded-xl p-6 text-center shadow-md hover:shadow-lg transition-all duration-300 flex flex-col items-center border border-gray-100"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          <motion.section
            ...
            viewport={{ once: true, amount: 0.2 }}
          >
            <h3 className="text-2xl font-bold text-green-700 mb-6">For Farmers</h3>
            <ul className="space-y-4">
              {[
                ...
                <motion.li
                  key={index}
                  className="flex items-start text-md text-gray-600"
                  variants={itemVariants}
          ...
          <motion.section
            ...
            viewport={{ once: true, amount: 0.2 }}
          >
            <h3 className="text-2xl font-bold text-green-700 mb-6">For Businesses</h3>
            <ul className="space-y-4">
              {[
        ...
        <motion.section
          ...
          viewport={{ once: true, amount: 0.2 }}
        >
          <h3 className="text-3xl font-bold text-green-800 text-center mb-10">The Cropzap CODE</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <motion.div
                ...
                viewport={{ once: true, amount: 0.4 }}
              >
                <div className="text-6xl font-extrabold text-green-600 mb-4">{item.letter}</div>
        ...
        <motion.section
          ...
          viewport={{ once: true, amount: 0.2 }}
        >
          <h3 className="text-3xl font-bold text-green-800 mb-4">Innovate with Cropzap</h3>
      ...
      <motion.button
        ...
        transition={{ type: "spring", stiffness: 200, damping: 20, delay: 1 }} // Spring animation for appearance
      ...
      <AnimatePresence>
        {showSellerAppPopup && (
          <motion.div
            className="fixed bottom-6 right-6 z-50 bg-white rounded-3xl shadow-xl border border-gray-100 p-6 w-full max-w-sm lg:max-w-md"
          ...
          >
            ...
            <div className="flex flex-col items-center text-center">
              <img
                src="/Screenshot 2025-06-13 at 8.39.17 PM.jpg" // Seller app image
                alt="Cropzap Seller App Icon"
                className="w-24 h-24 object-contain rounded-lg mb-4 shadow-md border border-gray-100"
              />
              ...
              <motion.a
                ...
                className="bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-8 rounded-full shadow-lg
                           ...
                           hover:scale-105 active:scale-95 transform group relative overflow-hidden text-md"
                whileHover={{ boxShadow: "0 10px 20px rgba(0, 128, 0, 0.2)" }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setShowSellerAppPopup(false)} // Close popup on download click
              >
                <span className="relative z-10">Download Now</span>
      ...
      {/* Footer */}
      <footer className="bg-gray-100 text-gray-600 py-6 text-center text-sm border-t border-gray-200">
